	...
	if(isset($_POST["submit"]))
		{
			if($_POST["submit"] == "Search Above Query 1")
				{
					$_SESSION["values"] = $_POST["Gu_input"];
					//echo $_POST["Gu_input"];
				}
			elseif($_POST["submit"] == "Search Above Query 2")
				{
					$_SESSION["values"] = $_POST["Gu_input_2"];
					//echo $_SESSION["value"];
				}
		}
	//header('Location: session.php')

	$Gujarati_word = $_SESSION["values"];	//Get the Gujarati value
	if($Gujarati_word == null)
	{
		$_SESSION["values"] = $_SESSION["pass_value_2"];
	}
	//echo $Gujarati_word;
...
<html lang="en">
  ...
  <body>
	<?php
		include 'header_page_After_translation.php';
	?>
    ...
    <div class="jumbotron">
      <div class="container" style="margin-left:15%;">
		 ...
		 ?>
			...
			<p>After Translator  : <font color="orange"><?php echo $_SESSION["values"];?></font>
			<a href ="suggestionpage.php">&nbsp;&nbsp;&nbsp;Suggest any new Meaning of this Word?</a></p>
			</p><br>
		 <?php 
				}
			else
				{
		 ?>
					<p>Given Input using virtual keyboard  is :<font color="orange">&nbsp; <?php if(isset($_SESSION["values"])){if($_SESSION["values"]){echo $_SESSION["values"];$Gujarati_word = $_SESSION["values"];}}?></font>
					<a href ="suggestionpage.php">&nbsp;&nbsp;&nbsp;Suggest Meaning of the Word?</a></p>
		<?php
				}
		?>
...
<?php
	...
	//echo "<br>".$num."<br>";
		...
		<p style="left:15;"><a href ="suggestionpage.php">&nbsp;&nbsp;&nbsp;Suggest Meaning of the Word?</a></p>
		<?php
							}
				}
	}
	else
	{
		...
		mysql_select_db($database,$connection);	//connect with database
		mysql_query("set names utf8");	//use Gujarati keyword
		$sql= "select English_meaning1,English_meaning2,English_meaning3 from keyword_meaning_2 where Gujarati_input = '$Gujarati_word' LIMIT 1";	//SQL Query
		$result= mysql_query($sql);
		$num= mysql_num_rows($result);
		if($num != 0)
				{
		...
		<font color="red">
		<?php
		echo "No Meaning is found Please Give suggestion so we improve our dictionary. ";
		?>
		</font>
			<a href ="suggestionpage.php">&nbsp;&nbsp;&nbsp;Suggest Meaning of the Word?</a>
		<?php
				}
	}
?>
